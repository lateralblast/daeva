...
def get_puppet_pkg_url(app_name,app_url)
  pkg_url = Net::HTTP.get(URI.parse(app_url)).split("\n").grep(/puppet/)[-1].split(/"/)[7]
  pkg_url = app_url+pkg_url
  return pkg_url
end

def get_puppet_rem_ver(app_name,app_url)
  rem_ver = get_puppet_pkg_url(app_name,app_url)
...
def do_puppet_post_install(app_name,app_url)
  return
end
